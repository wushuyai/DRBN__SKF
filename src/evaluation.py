...
import argparse
# import utils
from src.hrseg.hrseg_lib.utils import utils
...
def measure_dirs(dirA, dirB, use_gpu, verbose=False):
    ...
    print("Paths A:", paths_A)
    print("Paths B:", paths_B)
...
if __name__ == "__main__":
    ...
    # parser.add_argument('-dirA', default='/data1/xxxxxx/dataset/lowlight/LOL/test/high/', type=str)
    # parser.add_argument('-dirB', default='/data1/xxxxxx/project/DRBL-stage1/experiment/test_seg_hist_fuse_wo_pre_adv_lol/results/', type=str)
    parser.add_argument('-dirA', default=os.path.join('.', 'dataset', 'lowlight', 'LOL', 'test', 'high'), type=str)
    parser.add_argument('-dirB', default=os.path.join('.', 'project', 'DRBL-stage1', 'experiment','test_seg_hist_fuse_wo_pre_adv_lol', 'results'), type=str)
    ...
    if not os.path.exists(dirA):
        os.makedirs(dirA)
        print(f"Created directory {dirA}")

    # 检查并创建目录B
    if not os.path.exists(dirB):
        os.makedirs(dirB)
        print(f"Created directory {dirB}")
    type = args.type
