...
class Trainer():
    def __init__(self, args, loader, my_model, my_loss, ckp, adv=False):
        ...
        print(type(self.loader_test))  # 打印 self.loader_test 的类型
        self.model = my_model
        self.loss = my_loss
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            # print(type(self.loader_test))
                tqdm_test = tqdm(self.loader_test, ncols=80)
                # print("值",tqdm_test)
                # print("tqdm_test",len(tqdm_test))
                ...
                print("长度",len(self.loader_test))
                self.ckp.log[-1, idx_scale] = eval_acc / len(self.loader_test)
                # self.ckp.log[-1, idx_scale] = eval_acc / 2
...
# args = type('', (), {})()  # 创建一个空对象作为示例
# args.scale = [1, 2, 3]     # 添加 scale 属性
# loader = type('', (), {})() # 创建一个空对象作为示例
# loader.loader_train = []    # 添加 loader_train 属性
# loader.loader_test = []     # 添加 loader_test 属性
# ckp = None                  # 假设 ckp 是某个对象
# my_model = None             # 假设 my_model 是某个对象
# my_loss = None              # 假设 my_loss 是某个对象
#
# trainer = Trainer(args, loader, my_model, my_loss, ckp)
# trainer.process()